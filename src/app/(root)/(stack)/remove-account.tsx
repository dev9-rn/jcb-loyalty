...
const RemoveAccountScreen = ({ }: Props) => {
    ...
    const handleAccountDeletion = async () => {
        ...
        accountDeletionFormData.append("userType", userDetails?.userType);
        accountDeletionFormData.append("language", i18n.language);

        try {
            const response = await axiosInstance.post(DELETE_USER_ACCOUNT, accountDeletionFormData);
            ...
            <View className='gap-4 flex-1'>
                ...
                <View className='gap-2'>
                    ...
                    <View className='ml-5 gap-2'>
                        <Text>{'\u2B24'} {t('removeAccount.dataPoints.profileInfo')}</Text>
                        <Text>{'\u2B24'} {t('removeAccount.dataPoints.loginHistory')}</Text>
                        <Text>{'\u2B24'} {t('removeAccount.dataPoints.couponHistory')}</Text>
                    </View>
                </View>
                ...
                <View className='gap-4'>
                    <Text>
                        <Text className='font-semibold'>{t('removeAccount.warningTitle')} </Text>
                        {t('removeAccount.warningText')}
                    </Text>
                    <View className='gap-2'>
                        <Text>{'\u274C'} {t('removeAccount.warnings.noReRegister')}</Text>
                        <Text>{'\u274C'} {t('removeAccount.warnings.couponLoss')}</Text>
                    </View>
                </View>

                <View className='flex-row items-center gap-2 flex-shrink'>
                    <Checkbox checked={isConditionChecked} onCheckedChange={setIsConditionChecked} />
                    <Text>{t('removeAccount.checkboxLabel')}</Text>
                </View>

                <View className='flex-row items-center justify-around mt-auto'>
                    <RemoveAccountDialog isConditionChecked={isConditionChecked} handleAccountDeletion={handleAccountDeletion} />
