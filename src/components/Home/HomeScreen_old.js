        ...
        <Header style={{ backgroundColor: '#0000FF' }}>
          <Left style={{ flex: 0.1 }}>
            <TouchableOpacity onPress={() => { this.toggleControlPanel() }}>
              <Icon type="FontAwesome" name="bars" style={{ fontSize: 25, color: '#FFFFFF', paddingRight: 10 }} />
            </TouchableOpacity>
          </Left>
          <Body style={{ flex: 0.8, alignItems: 'center' }}>
            <Title style={{ color: '#FFFFFF', fontSize: 16 }}>{strings('login.dashboard_title')}</Title>
          </Body>
          <Right style={{ flex: 0.1 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
              <TouchableOpacity onPress={() => { this._onPressNotificationIcon() }}>
                <IconBadge
                  MainElement={
                    <Icon type="FontAwesome" name="bell" style={{ fontSize: 30, height: 35, width: 40, color: '#FFFFFF', margin: 7, marginBottom: 0 }} />
                  }
                  BadgeElement={
                    <Text style={{ color: '#FFFFFF', fontSize: 10 }}>{this.state.count}</Text>
                  }
                  IconBadgeStyle={
                    {
                      width: 25,
          ...
          <View style={{ flex: 1, padding: 15, marginTop: -10 }}>
            <ScrollView maximumZoomScale={1} keyboardShouldPersistTaps="handled">
              ...
              <View style={{ flex: 1, flexDirection: 'row' }}>
                <Card style={{ flex: 0.5, marginRight: 5, padding: 5 }}>
                  <Text style={{ height: this.props.languageControl == "Tamil - (தமிழ்)" ? 80 : 75, textAlign: 'center' }}>{strings('login.total_coupon_scan')}</Text>
                ...
                <Card style={{ flex: 0.5, marginLeft: 5, padding: 5 }}>
                  <Text style={{ height: this.props.languageControl == "Tamil - (தமிழ்)" ? 80 : 75, textAlign: 'center' }}>{strings('login.total_redemption_amount_till_d')}</Text>
                  <Text style={{ padding: 20, textAlign: 'center', fontSize: 24 }}>{this.state.totalAmountRedeemed}</Text>
                </Card>
              </View>
              ...
              <View style={{ flex: 1, flexDirection: 'row' }}>
                <Card style={{ flex: 0.5, marginRight: 5, padding: 5 }}>
                  <Text style={{ height: this.props.languageControl == "Tamil - (தமிழ்)" ? 80 : 75, textAlign: 'center' }}>{strings('login.total_coupons_scanned_for_cash')}</Text>
                  <Text style={{ padding: 20, textAlign: 'center', fontSize: 24 }}>{this.state.totalCouponsRedeemedCash}</Text>
                </Card>
              ...
              <View style={{ marginTop: 50, width: 150, justifyContent: 'center', flex: 1, alignSelf: 'center' }}>
                {/* <Button onPress={this._onPressScanButton} title="SCAN" />
                {
                  !this.state.btnScanEnabled ?
                  <Text  style={ {color:'blue'}}>Scan again in { this.state.time } </Text> :
                  <View></View>
                }
                <Button style={{ backgroundColor: this.state.btnScanEnabled ? 'blue' : '#ccc', borderRadius: 20 }}
                         onPress={ this.state.btnScanEnabled ? this._onPressScanButton : null } > */}
                <Button style={{ backgroundColor: 'blue', borderRadius: 20 }} onPress={this._onPressScanButton}>
                  <Text style={{ textAlign: 'center', flex: 1, fontWeight: 'bold', fontSize: 18 }}>{strings('login.scan_button')}</Text></Button>
              </View>
