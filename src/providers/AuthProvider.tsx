...
import { setMaintenanceHandler } from "@/libs/maintenanceHandler";
...
const AuthProvider = ({ children }: Props) => {
    ...
    const getLocalUser = () => {
        ...
        const localAuthToken = tokenStorageService.getAuthToken(
            STORAGE_KEYS.AUTH_TOKEN,
        );
        const localUserColorScheme = storageService.getItem(
    ...
    const login = async (endpoint: string, formData: FormData) => {
        try {
    ...
    const verify = async (endpoint: string, verifyFormData: FormData) => {
        try {
            ...
            if (response.data.data?.is_approved != "0") {
                setUserAuthToken(
                    response.data?.data?.accesstoken || response.headers.accesstoken,
                );
    ...
    const fetchisMaintenanceApi = async (): Promise<void> => {
        try {
            const { data } = await axiosInstance.get<IFMaintenance>(IS_MAINTENANCE, {
                headers: { skipMaintenance: true },
            });

            setMaintenanceMsg(data?.systemStatus);

            if (data?.maintenanceStatus > 0) {
